...
def test_only_superadmin_can_change_user_role(client, superadmin_token, teacher_admin_token, fixed_users):
    """
    admin_update_user (cambiar rol teacher <-> teacher_admin) usa
...
def test_withdrawals_pending_real_path(client, superadmin_token):
    """
    Ruta real (el tester viejo llamaba /api/v1/admin/withdrawals/pending,
    que no existe bajo ese router — 404 encubierto como 'no funciona').
    """
...
def test_admin_classes_list_endpoint_does_not_exist(client, superadmin_token):
    """
    Documenta explícitamente que /api/v1/admin/classes (usado por el tester
    viejo como 'cls-admin-list') no existe en este backend — no hay listado
    global de clases para admin, solo por profesor/estudiante/cohorte.
    Si algún día se agrega, este test empezará a fallar y habrá que
    actualizar/():eliminar esta nota.
    """
