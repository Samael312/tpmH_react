...
function PackageCard({ pkg, showTeacher }: { pkg: LandingPackage; showTeacher: boolean }) {
...
// Con pocos paquetes (hasta 4, lo que suele entrar cómodo en una fila) un
// carrusel de scroll horizontal se ve raro: quedan pegados a la izquierda
// con un montón de espacio vacío a la derecha. A partir de ahí sí conviene
// el carrusel para poder navegar sin que la sección crezca sin límite.
const MAX_PACKAGES_WITHOUT_SCROLL = 4;
...
export default function PackagesCarousel({
  ...
  const cards = packages.map((pkg) => (
    <PackageCard key={pkg.id} pkg={pkg} showTeacher={showTeacher} />
  ));

  if (packages.length <= MAX_PACKAGES_WITHOUT_SCROLL) {
    return <div className="flex flex-wrap justify-center gap-5">{cards}</div>;
  }

  return <Carousel ariaLabel="Paquetes disponibles">{cards}</Carousel>;
}